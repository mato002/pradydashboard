...
>
    ...
    <div
        ...
        @click.stop
    >
        <h2 id="prady-command-palette-title" class="sr-only">{{ __('Global feature finder') }}</h2>
        ...
        <div class="border-b border-slate-200 px-4 py-3 dark:border-slate-700">
            <div class="relative">
                ...
                <input
                    ...
                    placeholder="{{ __('Search tenants, invoices, servers, settings, features…') }}"
                    autocomplete="off"
                    aria-label="{{ __('Search dashboard features') }}"
                >
        ...
        <div class="max-h-[min(60vh,28rem)] overflow-y-auto py-2" x-ref="paletteResultsPanel">
                    ...
                    <template x-for="(item, index) in recentItems" :key="`recent-${item.id}`">
                        ...
                        >
                            ...
                            <div class="flex shrink-0 items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                                <button type="button" class="rounded-lg p-1.5 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800" @click="openPaletteItemNewTab(item)" title="{{ __('Open in new tab') }}">
                                    <x-ui.icon name="up-right-from-square" class="h-3.5 w-3.5" />
                                </button>
                                <button type="button" class="rounded-lg p-1.5 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800" @click="copyPaletteItemLink(item)" title="{{ __('Copy link') }}">
                                    <x-ui.icon name="clipboard" class="h-3.5 w-3.5" />
                                </button>
                                <button type="button" class="rounded-lg p-1.5 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800" @click="toggleDiscoveryFavorite(item.id)" title="{{ __('Favorite') }}">
            ...
            >
                {{ __('Search is temporarily unavailable. Refresh the page and try again.') }}
            </p>
        ...
        <div class="flex items-center justify-between border-t border-slate-200 bg-slate-50/80 px-4 py-2 text-[11px] text-slate-500 dark:border-slate-700 dark:bg-slate-950/60">
            <div class="flex flex-wrap items-center gap-3">
                <span class="inline-flex items-center gap-1"><kbd class="rounded border border-slate-200 px-1 dark:border-slate-600">↑</kbd><kbd class="rounded border border-slate-200 px-1 dark:border-slate-600">↓</kbd> {{ __('Navigate') }}</span>
                <span class="inline-flex items-center gap-1"><kbd class="rounded border border-slate-200 px-1 dark:border-slate-600">↵</kbd> {{ __('Open') }}</span>
                <span class="inline-flex items-center gap-1"><kbd class="rounded border border-slate-200 px-1 dark:border-slate-600">Esc</kbd> {{ __('Close') }}</span>
            </div>
