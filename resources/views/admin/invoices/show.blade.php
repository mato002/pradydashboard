...
<x-dashboard-layout :heading="$invoice->invoice_number" :subheading="$subheading">
    <div class="space-y-6">
        @if (session('status'))
        ...
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-xs text-gray-500">
                    @if ($isStatement && $invoice->statement_period_start)
                        {{ __('Period') }}: {{ $invoice->statement_period_start->toFormattedDateString() }} — {{ optional($invoice->statement_period_end)->toFormattedDateString() ?? '—' }}
                    @else
                        {{ __('Due') }} {{ optional($invoice->due_date)->toFormattedDateString() ?? '—' }}
                    @endif
                </p>
                @if ($isReceipt && $invoice->linkedInvoice)
                    <p class="mt-1 text-xs text-slate-600 dark:text-slate-300">
                        {{ __('Invoice ref') }}:
                        <a href="{{ route('invoices.show', $invoice->linkedInvoice) }}" class="text-indigo-600 hover:underline">{{ $invoice->linkedInvoice->invoice_number }}</a>
                        @if ($invoice->linkedInvoice->isCancelled())
                            <span class="text-amber-700 dark:text-amber-300">({{ __('source cancelled') }})</span>
                        @endif
                    </p>
                @endif
                @if ($invoice->convertedInvoice)
                    <p class="mt-1 text-xs text-slate-600 dark:text-slate-300">
                        {{ __('Converted to') }}:
                        <a href="{{ route('invoices.show', $invoice->convertedInvoice) }}" class="text-indigo-600 hover:underline">{{ $invoice->convertedInvoice->invoice_number }}</a>
                    </p>
                @endif
            </div>
            <div class="flex flex-wrap gap-2">
                ...
                @if ($isQuotation && $invoice->canConvert())
                ...
                @if ($isProforma && $invoice->canConvert())
                ...
                @if ($isInvoice && $invoice->balanceDue() <= 0.009 && $invoice->status !== 'paid')
        ...
        @if ($isInvoice && $invoice->balanceDue() > 0.009 && ! in_array($invoice->status, ['paid', 'cancelled', 'void', 'draft']))
            ...
            <div class="grid gap-6 lg:grid-cols-3">
                <div class="lg:col-span-2 rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <div class="border-b border-gray-200 px-4 py-3 dark:border-gray-800">
                        <h3 class="text-sm font-semibold">{{ $isReceipt ? __('Payment') : __('Line items') }}</h3>
                    </div>
                ...
                <div class="space-y-4">
                    ...
                    @if ($isInvoice && $invoice->payments->isNotEmpty())
                        <div class="rounded-xl border border-gray-200 p-4 text-sm dark:border-gray-800">
