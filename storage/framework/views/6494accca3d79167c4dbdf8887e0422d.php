    ...
    >
        ...
        <?php if($selectedGatewayLabel): ?>
            <div class="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-indigo-500/25 bg-indigo-500/5 px-5 py-3 ring-1 ring-indigo-500/15">
                <p class="text-sm font-medium text-slate-700 dark:text-slate-200">
                    <?php echo e(__('Showing transactions for :gateway.', ['gateway' => $selectedGatewayLabel])); ?>

                </p>
                <a
                    href="<?php echo e(route('payments.index', array_filter(['status' => request('status')]))); ?>#ledger"
                    class="text-sm font-semibold text-indigo-600 hover:text-indigo-500 dark:text-indigo-400"
                ><?php echo e(__('Clear gateway filter')); ?></a>
            </div>
        <?php endif; ?>
            ...
            <div class="lg:col-span-8">
                <div class="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-card dark:border-slate-800/80 dark:bg-slate-900/60">
                    <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200/80 px-4 py-3 dark:border-slate-800/80">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <?php $__currentLoopData = ['' => __('All'), 'successful' => __('Successful'), 'pending' => __('Pending'), 'failed' => __('Failed'), 'refunded' => __('Refunded')]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $val => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <a
                                    href="<?php echo e(route('payments.index', array_filter(['status' => $val ?: null, 'gateway' => request('gateway')]))); ?>#ledger"
                    ...
                    <div class="flex flex-wrap gap-2 border-b border-slate-200/80 px-4 py-2.5 dark:border-slate-800/80">
                        <span class="self-center text-[10px] font-semibold uppercase tracking-wide text-slate-400"><?php echo e(__('Gateway')); ?></span>
                        <?php $__currentLoopData = $gateways; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $gw): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            ...
                            ?>
                            <a
                                href="<?php echo e(route('payments.index', array_filter([
...
<?php endif; ?>
                                        ...
                                        <td class="text-right">
                                            <div class="inline-flex gap-1 opacity-70 transition group-hover:opacity-100">
                                                <button type="button" title="<?php echo e(__('View Receipt')); ?>" class="rounded-lg p-1.5 text-slate-500 hover:bg-slate-100 hover:text-emerald-600 dark:hover:bg-slate-800">
                                                ...
                                                <button type="button" title="<?php echo e(__('Allocate')); ?>" class="rounded-lg p-1.5 text-slate-500 hover:bg-slate-100 hover:text-indigo-600 dark:hover:bg-slate-800">
                                                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M7.5 21 3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5" /></svg>
                                                </button>
                                                <button type="button" title="<?php echo e(__('Refund')); ?>" class="rounded-lg p-1.5 text-slate-500 hover:bg-slate-100 hover:text-violet-600 dark:hover:bg-slate-800">
                                ...
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                                    <tr>
                                        <td colspan="9" class="py-12 text-center text-sm text-slate-500">
                                            <?php if($selectedGatewayLabel): ?>
                                                <?php echo e(__('No :gateway transactions match the current filters.', ['gateway' => $selectedGatewayLabel])); ?>

                                            <?php else: ?>
                                                <?php echo e(__('No payment transactions recorded yet.')); ?>
            ...
            <div class="grid gap-4 p-4 sm:grid-cols-2 lg:grid-cols-5">
                <?php $__currentLoopData = $gateways; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $gw): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    ...
                    <a
                        ...
                        aria-current="<?php echo e($gwActive ? 'true' : 'false'); ?>"
                    >
                        ...
                        <div class="mt-3 space-y-2">
                            <div>
                                <div class="mb-0.5 flex justify-between text-[10px]">
                                    ...
                                    <span class="font-semibold tabular-nums"><?php echo e($uptime !== null ? $uptime.'%' : '—'); ?></span>
                                </div>
                                <div class="h-1 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
                                    <div class="h-full rounded-full bg-emerald-500" style="width: <?php echo e(min(100, max(0, (float) $uptime))); ?>%"></div>
                                </div>
                            </div>
                            <div>
                                <div class="mb-0.5 flex justify-between text-[10px]">
                                    <span class="text-slate-500"><?php echo e(__('Success rate')); ?></span>
                                    <span class="font-semibold tabular-nums"><?php echo e($success !== null ? $success.'%' : '—'); ?></span>
                                </div>
                                <div class="h-1 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
                                    <div class="h-full rounded-full bg-indigo-500" style="width: <?php echo e(min(100, max(0, (float) $success))); ?>%"></div>
                                </div>
                            </div>
                            <p class="text-[10px] text-slate-400"><?php echo e(__('Latency')); ?>: <span class="font-mono font-semibold text-slate-600 dark:text-slate-300"><?php echo e(filled($gw['latency']) ? $gw['latency'].'ms' : '—'); ?></span></p>
                            <?php if($gwActive): ?>
                                <p class="mt-2 text-[10px] font-semibold uppercase tracking-wide text-indigo-600 dark:text-indigo-400"><?php echo e(__('Filtering ledger')); ?></p>
                            <?php endif; ?>
                        </div>
                    </a>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\prady-dashboard\resources\views/admin/payments/index.blade.php ENDPATH**/ ?>