...
<div class="mb-6 space-y-4">
    <?php $__currentLoopData = $metricGroups; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $group): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <section>
            <h3 class="mb-2 text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400"><?php echo e($group['label']); ?></h3>
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
                <?php $__currentLoopData = $group['items']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php $tone = $toneClasses[$item['tone']] ?? $toneClasses['neutral']; ?>
                    <div class="rounded-2xl border p-3.5 shadow-sm <?php echo e($tone); ?>">
                        <div class="flex items-start gap-2.5">
                            <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-white/60 dark:bg-black/20">
                                <svg class="h-4 w-4 opacity-80" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="<?php echo e($iconPaths[$item['icon']] ?? $iconPaths['grid']); ?>" />
                                </svg>
                            </span>
                            <div class="min-w-0">
                                <p class="text-[11px] font-medium leading-snug text-slate-600 dark:text-slate-300"><?php echo e($item['label']); ?></p>
                                <p class="mt-1 text-sm font-semibold leading-snug tabular-nums"><?php echo e($item['value']); ?></p>
                            </div>
                        </div>
                    </div>
    ...
    <?php if($opsSummary['public_url'] ?? null): ?>
        <p class="truncate text-xs text-indigo-600 dark:text-indigo-400">
            <a href="<?php echo e($opsSummary['public_url']); ?>" target="_blank" rel="noopener noreferrer"><?php echo e($opsSummary['public_url']); ?></a>
        </p>
    <?php endif; ?>
</div>
<?php /**PATH C:\Users\HP\Desktop\Newfolder\pradydashboard\resources\views/admin/tenants/partials/workspace/metrics.blade.php ENDPATH**/ ?>