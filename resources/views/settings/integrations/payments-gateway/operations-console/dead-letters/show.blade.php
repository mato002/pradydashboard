<x-dashboard-layout :heading="__('Payments Gateway')" :subheading="__('Dead letter investigation')">
    <div class="space-y-6">
        ...
        @if ($resource)
            ...
            <x-admin.form-shell
                :title="__('Dead letter :uuid', ['uuid' => substr($uuid, 0, 8).'…'])"
                :subtitle="($resource['queue'] ?? $resource['queue_name'] ?? $uuid)"
                ...
                <div class="grid gap-4 lg:grid-cols-3">
                    <div class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-card dark:border-slate-800 dark:bg-slate-900/60 lg:col-span-2">
                        <div class="flex items-start justify-between gap-3">
                            <h3 class="text-sm font-semibold text-slate-900 dark:text-white">{{ __('Dead letter identity') }}</h3>
                        ...
                        <dl class="mt-4 grid gap-3 sm:grid-cols-2">
                            ...
                            @include('settings.integrations.payments-gateway.partials.detail-field', ['label' => __('Type'), 'value' => $resource['type'] ?? $resource['source_type'] ?? '—'])
                            @include('settings.integrations.payments-gateway.partials.detail-field', ['label' => __('Queue'), 'value' => $resource['queue'] ?? $resource['queue_name'] ?? '—'])
                            @include('settings.integrations.payments-gateway.partials.detail-field', ['label' => __('Related type'), 'value' => $resource['related_type'] ?? '—'])
                            @include('settings.integrations.payments-gateway.partials.detail-field', ['label' => __('Related UUID'), 'value' => $resource['related_uuid'] ?? '—'])
                            @include('settings.integrations.payments-gateway.partials.detail-field', ['label' => __('Payment transaction UUID'), 'value' => $resource['payment_transaction_uuid'] ?? '—'])
                            @include('settings.integrations.payments-gateway.partials.detail-field', ['label' => __('Callback log UUID'), 'value' => $resource['callback_log_uuid'] ?? '—'])
                            @include('settings.integrations.payments-gateway.partials.detail-field', ['label' => __('Webhook event UUID'), 'value' => $resource['webhook_event_uuid'] ?? '—'])
                            @include('settings.integrations.payments-gateway.partials.detail-field', ['label' => __('Webhook delivery UUID'), 'value' => $resource['webhook_delivery_uuid'] ?? '—'])
                            @include('settings.integrations.payments-gateway.partials.detail-field', ['label' => __('Failure reason'), 'value' => $resource['failure_reason'] ?? '—'])
