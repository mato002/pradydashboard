    ...
    <ul class="divide-y divide-gray-200 dark:divide-gray-800">
        @forelse ($supportSummary['open_tickets'] as $ticket)
            <li class="flex flex-wrap items-center justify-between gap-2 px-4 py-3 text-sm">
                ...
                @if ($ticket->tenant)
                    <a href="{{ route('tenants.show', ['tenant' => $ticket->tenant, 'tab' => 'support', 'ticket' => $ticket->public_id]) }}" class="text-xs text-gray-500 hover:text-indigo-600">{{ $ticket->tenant->company_name }}</a>
                @endif
            </li>
        @empty
