...
<aside class="relative hidden min-h-[280px] flex-col overflow-hidden bg-enterprise-mesh lg:flex lg:min-h-0 lg:w-[52%] lg:shrink-0 xl:w-[50%]">
    ...
    <div class="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden="true">
        <?php $__currentLoopData = [12, 28, 45, 62, 78, 88, 22, 55, 70, 35, 8, 92, 18, 48, 65, 82, 38, 72, 5, 58, 25, 42, 75, 15]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $left): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <span
                class="absolute h-1 w-1 rounded-full bg-cyan-300/60 animate-auth-particle"
                style="left: <?php echo e($left); ?>%; top: <?php echo e(15 + ($i * 3.2) % 70); ?>%; animation-delay: <?php echo e($i * 0.35); ?>s; animation-duration: <?php echo e(5 + ($i % 4)); ?>s"
            ></span>
    ...
    <div class="relative z-[1] flex flex-1 flex-col justify-between gap-6 px-6 py-8 sm:px-8 sm:py-9 lg:px-10 lg:py-10 xl:px-12">
        <div class="animate-auth-fade-up">
            <a href="<?php echo e(route('home')); ?>" class="inline-flex items-center gap-3 transition-opacity hover:opacity-90">
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.brand-logo','data' => ['class' => 'h-7 w-7 text-cyan-300']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['class' => 'h-7 w-7 text-cyan-300']); ?>
...
<?php endif; ?>
                ...
                <span>
                    ...
                    <span class="block text-base font-bold tracking-tight text-white"><?php echo e(__('Operations Cloud')); ?></span>
                </span>
            </a>
            <span class="mt-4 inline-flex items-center gap-2 rounded-full border border-cyan-400/25 bg-cyan-500/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-cyan-200 ring-1 ring-cyan-400/20">
                ...
                <?php echo e(__('Enterprise Infrastructure Platform')); ?>
        ...
        <div class="flex flex-1 flex-col gap-5 lg:gap-6 animate-auth-fade-up-delay">
            <div>
                <h1 class="max-w-xl text-2xl font-extrabold leading-[1.12] tracking-tight text-white sm:text-[1.75rem] xl:text-[2rem]">
                    <?php echo e(__('Operate your SaaS infrastructure from one intelligent control plane.')); ?>

                </h1>
                <p class="mt-3 max-w-xl text-[13px] leading-relaxed text-slate-300/90 sm:text-sm">
                    <?php echo e(__('Orchestrate tenants, monitor infrastructure health, ship deployments, manage SSL lifecycles, automate billing, enforce access control, and maintain full observability—from a single enterprise-grade operations layer built for scale.')); ?>
            ...
            <ul class="grid max-w-xl gap-0.5 sm:grid-cols-2 sm:gap-x-4">
                <?php $__currentLoopData = $features; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $feature): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <li class="auth-feature-row group">
                        ...
                        <span class="leading-snug"><?php echo e($feature['label']); ?></span>
                    </li>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </ul>
        </div>
        ...
        <div class="animate-auth-fade-up-delay-2">
            <div class="mb-3 flex items-center justify-between gap-2">
                <p class="text-[10px] font-bold uppercase tracking-[0.22em] text-slate-400"><?php echo e(__('Live platform telemetry')); ?></p>
                <span class="inline-flex items-center gap-1.5 text-[10px] font-semibold text-emerald-300">
                    <span class="relative flex h-1.5 w-1.5">
                        <span class="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-400 opacity-60"></span>
                        <span class="relative inline-flex h-1.5 w-1.5 rounded-full bg-emerald-400"></span>
                    </span>
                    <?php echo e(__('Operational')); ?>

                </span>
            </div>
            <div class="grid grid-cols-2 gap-2 sm:grid-cols-4 sm:gap-2.5">
                <?php $__currentLoopData = $metrics; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $metric): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="auth-metric-tile group" style="animation-delay: <?php echo e($i * 0.4); ?>s">
                        <p class="text-[9px] font-semibold uppercase tracking-wider text-slate-400 leading-tight"><?php echo e($metric['label']); ?></p>
                        <p
                            class="mt-1 font-mono text-base font-bold tabular-nums text-white sm:text-lg"
                            x-data="liveMetric(<?php echo e($metric['base']); ?>, <?php echo e($metric['variance']); ?>, '<?php echo e($metric['suffix']); ?>', <?php echo e($metric['decimals']); ?>)"
                            x-text="display"
                        >—</p>
                    </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
            <p class="mt-2.5 text-[10px] text-slate-500"><?php echo e(__('Illustrative metrics for demo environments')); ?></p>
        </div>
    </div>
</aside>
<?php /**PATH C:\Users\HP\Desktop\Newfolder\pradydashboard\resources\views/components/auth/partials/enterprise-panel.blade.php ENDPATH**/ ?>